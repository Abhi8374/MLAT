...
def save_checkpoint(checkpoint_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    path = '/content/Code/MLAT/models_checkpoint3'
    if os.path.exists(path):
      pass
    else:
      os.mkdir(path)
    filepath = os.path.join('/content/Code/MLAT'+'/models_checkpoint3/', filename)  # 最终参数模型
    torch.save(state, filepath)

    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, os.path.join('/content/Code/MLAT'+'/models_checkpoint3/', 'BEST_' + filename))

    torch.save(state, os.path.join('/content/Code/MLAT'+'/models_checkpoint3/', 'epoch_'+str(epoch) + filename))
